package online.incc.service.impl;

import java.io.File;
import java.io.IOException;
import java.util.Date;
import java.util.List;

import javax.annotation.Resource;

import org.springframework.cache.annotation.Cacheable;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Propagation;
import org.springframework.transaction.annotation.Transactional;

import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;

import freemarker.template.TemplateException;
import online.incc.mapper.NewsMapper;
import online.incc.model.News;
import online.incc.model.Product;
import online.incc.service.NewsService;
import online.incc.util.GenerateHtml;
import online.incc.util.RealPath;
import online.incc.vo.NewsVO;
import tk.mybatis.mapper.entity.Example;

@Service("newsService")
public class NewsServiceImpl extends BaseService<News> implements NewsService {
	@Resource
	private NewsMapper newsMapper;

	@Override
	@Cacheable(cacheNames="newsList",key="#start")
	public PageInfo<News> selectByPage(News News, int start, int length) {
		int page = start / length + 1;
		Example example = new Example(Product.class);
		Example.Criteria criteria = example.createCriteria();
		PageHelper.startPage(page, length);
		List<News> list = selectByExample(example);
		return new PageInfo<>(list);
	}

	@Override
	@Cacheable(cacheNames="news",key="#key")
	public News selectByKey(Integer key) {
		return super.selectByKey(key);
	}

	@Override
	@Transactional(propagation= Propagation.REQUIRED,readOnly=false,rollbackFor={Exception.class})
	public void add(NewsVO newsVO, String realPath) throws IOException, TemplateException {
		Date d = new Date();
		String dateStr = d.toLocaleString();
		newsVO.setDate(dateStr);
		Integer userId = userId();
		String realBasePath = RealPath.getRealPath(realPath);
		String newsPath = realBasePath;
		
		News news = new News();
		news.setDate(dateStr);
		news.setTitle(newsVO.getTitle());
		newsMapper.insertUseGeneratedKeys(news);
		news.setLink(news.getId()+".html");
		newsMapper.updateByPrimaryKeySelective(news);
		
		
		File htmlPage = new File(newsPath, news.getId()+".html");  
        
        if (!htmlPage.exists()) {     
                    // 生成静态展示页面  
                    String templateName = "news.html";  
                    String htmlName = news.getId()+".html";  
                    GenerateHtml.generate(newsPath,"html/news",newsVO, templateName, htmlName);  
                }     
		
	}

}
